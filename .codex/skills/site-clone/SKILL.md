---
name: site-clone
description: "Reverse-engineer and replicate any website into a pixel-perfect Next.js codebase using Playwright MCP for browser automation. Extracts design tokens, assets, interactions, and content, then dispatches parallel builder agents to reconstruct each section."
argument-hint: "<url1> [<url2> ...]"
user-invocable: true
---

# Site Clone (Codex + Playwright Edition)
...
This skill uses **Playwright MCP** for all browser automation. When the skill says "via browser MCP", use these Playwright operations:

| Operation | How to do it with Playwright MCP |
|-----------|--------------------------------|
| **Navigate to URL** | Use the Playwright `navigate` tool with the target URL |
| **Take screenshot** | Use the Playwright `screenshot` tool. Save the result to `docs/design-references/` |
| **Execute JavaScript** | Use the Playwright `evaluate` tool with JS code. This runs `page.evaluate()` — you can call `document.querySelector()`, `getComputedStyle()`, `window.scrollTo()`, etc. |
| **Click element** | Use the Playwright `click` tool with a CSS selector |
| **Hover element** | Use the Playwright `hover` tool with a CSS selector |
| **Scroll page** | Execute JS: `window.scrollTo(0, <y>)` via `evaluate` |
| **Resize viewport** | Use the Playwright `setViewportSize` tool with `{ width: 1440, height: 900 }` |
| **Get page text** | Execute JS: `document.body.innerText` via `evaluate` |
| **Wait** | Execute JS: `await new Promise(r => setTimeout(r, <ms>))` via `evaluate`, or use Playwright's `waitForTimeout` |

**All JavaScript snippets in this skill** (getComputedStyle, DOM traversal, asset discovery, etc.) should be executed via Playwright's `evaluate` tool. They run in the browser page context and return JSON results.

**Screenshot workflow:**
1. Navigate to URL
2. Set viewport to 1440x900 (desktop)
3. Take full-page screenshot → save to `docs/design-references/{page}-desktop.png`
4. Set viewport to 390x844 (mobile)
5. Take full-page screenshot → save to `docs/design-references/{page}-mobile.png`
...
1. **Playwright MCP is mandatory.** Verify Playwright MCP is available by attempting a simple navigate operation. If it fails, tell the user: "Please add Playwright MCP to your Codex config: add `npx @playwright/mcp@latest` as an MCP server." This skill cannot run without browser control.
2. Parse `$ARGUMENTS` as one or more URLs. Validate each — if invalid, ask the user. Navigate to each via Playwright and confirm it loads.
